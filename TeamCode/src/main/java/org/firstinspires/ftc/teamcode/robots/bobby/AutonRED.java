    ...
    private void executeAuton() {
        switch (autonState) {
            ...
            case LAUNCH:
                ...
                {
                    shootNum ++;
                    autonState = ( (shootNum == 1) ? State.BACK_UP_MORE :  State.STRAFE_TO_EXIT );
                }
                break;


            case BACK_UP_MORE:
                if(driveBackwards( secondBackUpDist, 0.4))
                {
                    turnTimer.reset();
                    autonState = State.TURN_TO_INTAKE;
                }
                break;
            ...
            case TURN_TO_LAUNCH:
                if(pidTurnToHeading(Math.toRadians(launchHeading))||turnTimer.milliseconds() >= maxTurnTime) autonState = State.FORTH_TO_LAUNCH;
                break;


            case FORTH_TO_LAUNCH:
                if(driveForward(secondBackUpDist, 0.5)) autonState = State.LAUNCH;
                break;
